...
vortex_length_err = Inf
vortex_distance_err = Inf
energies = Float64[]
tsf = TimeSeriesFile()
...
while iter.t < 10.0
    ...
    local v²_max = maximum(iter.vs) do vs
        maximum(v -> sum(abs2, v), vs)
    end
    local vortex_distance = (iter.fs[2][1] - iter.fs[1][1]).x
    global vortex_length_err = abs(Lvort - vortex_length_equilibrium) / vortex_length_equilibrium
    global vortex_distance_err = abs(vortex_distance - vortex_distance_equilibrium) / vortex_distance_equilibrium
    push!(energies, E)
    # @show nstep, t, vortex_length_err, E, v²_max / E, vortex_distance_err
    vortex_distance_err < 0.01 && break
end

@testset "Minimum energy state" begin
