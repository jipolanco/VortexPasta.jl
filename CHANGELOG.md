...
- Fix multi-GPU issue on AMDGPU.
  ...
  reason, we now resize arrays in the same device where they were initially created.
...
- Rename module: `Containers` -> `VectorsOfVectors`.

- Split CPU and GPU implementations of energy spectrum.
  Both should be considerably faster than before.

### Fixed

- This version hopefully fixes garbage values sometimes found in energy spectra
  (quite randomly, very difficult to reproduce).

## [0.32.12] - 2025-12-21

### Added

- Timestepping: add `step_refinement` and `step_reconnect` options.
...
- Timestepping: use `:greedy` scheduler in parallelisation of filament refinement.
...
- Reformulate computation of `FourierBandForcingBS` when `modify_length = false`.
  We now explicitly write the forcing velocity as aligned with the binormal vector, while
  previously we removed the component aligned with curvature using the Gram-Schmidt process.
  The end result is the same as before. Docs on the used formulation have been also added.
...
- Optimise computation of `BiotSavart.add_local_integrals!` (computed on CPU when
  `lia_segment_fraction` is set).
