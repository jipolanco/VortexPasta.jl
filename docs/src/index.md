...
For the tutorials, we may also need the [StaticArrays.jl](https://github.com/JuliaArrays/StaticArrays.jl) package, which speeds-up certain operations involving small arrays of static size (i.e., such that their size is known at compile time):

```julia
(simulation) pkg> add StaticArrays
```
