...
!!! warning "Zero mean vorticity condition"
    ...
    in a rotating reference frame with rotation rate ``\bm{Ω} = -2 \bm{ω}_0``
    (that is, rotating in the same direction as the vortices).
...
!!! note "Size of FFTs"
    ...
    As an example, by default the [`NonuniformFFTsBackend`](@ref) in VortexPasta uses an oversampling of ``σ = 3/2``.
    Therefore, choosing ``N = 85 ≈ 2/3 × 128`` leads to FFTs of size ``Ñ = 128 = 2^7``, which is what we want.
