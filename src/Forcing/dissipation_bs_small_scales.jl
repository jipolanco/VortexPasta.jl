...
Compared to [`DissipationBS`](@ref), which in principle acts at all scales,
`SmallScaleDissipationBS` acts mainly at small scales (wavenumbers `k > kdiss`). It is also
much more expensive to compute, since it requires a low-pass filter done in Fourier space.
